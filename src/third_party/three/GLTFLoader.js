...
export default (function() {
  ...
  GLTFMaterialsUnlitExtension.prototype.extendParams = function(
      materialParams, material, parser) {
    var pending = [];
  ...
  GLTFDracoMeshCompressionExtension.prototype.decodePrimitive = function(
    ...
    return parser.getDependency('bufferView', bufferViewIndex)
    ...
    return {
      ...
      extendParams: function(params, material, parser) {
        ...
        if (pbrSpecularGlossiness.diffuseTexture !== undefined) {
          pending.push(parser.assignTexture(
              params, 'map', pbrSpecularGlossiness.diffuseTexture));
        }
      ...
      createMaterial: function(params) {
        // setup material properties based on MeshStandardMaterial for
        // Specular-Glossiness
      ...
      /**
       * Clones a GLTFSpecularGlossinessMaterial instance. The
       * ShaderMaterial.copy() method can copy only properties it knows about or
       * inherits, and misses many properties that would normally be defined by
       * MeshStandardMaterial.
       *
       * This method allows GLTFSpecularGlossinessMaterials to be cloned in the
       * process of loading a glTF model, but cloning later (e.g. by the user)
       * would require these changes AND also updating `.onBeforeRender` on the
       * parent mesh.
       *
      ...
      // Here's based on refreshUniformsCommon() and refreshUniformsStandard()
      // in WebGLRenderer.
      refreshUniforms: function(
        ...
        if (material.envMap) {
          ...
          //  WebGLRenderTargetCube.texture will be flipped because it's a
          ...
          uniforms.maxMipLevel.value =
              renderer.properties.get(material.envMap).__maxMipLevel;
        }
  ...
  GLTFCubicSplineInterpolant.prototype.copySampleValue_ = function(index) {
    ...
    var result = this.resultBuffer, values = this.sampleValues,
        valueSize = this.valueSize, offset = index * valueSize * 3 + valueSize;
  ...
  var INTERPOLATION = {
    CUBICSPLINE:
        THREE.InterpolateSmooth,  // We use custom interpolation
                                  // GLTFCubicSplineInterpolation for
                                  // CUBICSPLINE. KeyframeTrack.optimize() can't
                                  // handle glTF Cubic Spline output values
                                  // layout, using THREE.InterpolateSmooth for
                                  // KeyframeTrack instantiation to prevent
                                  // optimization. See KeyframeTrack.optimize()
                                  // for the detail.
  ...
  function updateMorphTargets(mesh, meshDef) {
    ...
    // .extras has user-defined data, so check that .extras.targetNames is an
    // array.
  ...
  function getCachedMultiPassGeometry(cache, geometry, primitives) {
    for (var i = 0, il = cache.length; i < il; i++) {
      ...
      if (geometry === cached.baseGeometry &&
  ...
  /**
   * Checks if we can build a single Mesh with MultiMaterial from multiple
   * primitives. Returns true if all primitives use the same
   * attributes/morphAttributes/mode and also have index. Otherwise returns
   * false.
   *
  ...
  GLTFParser.prototype.loadAccessor = function(accessorIndex) {
    ...
    if (accessorDef.sparse !== undefined) {
      pendingBufferViews.push(this.getDependency(
          'bufferView', accessorDef.sparse.indices.bufferView));
      pendingBufferViews.push(this.getDependency(
          'bufferView', accessorDef.sparse.values.bufferView));
    }
    ...
    return Promise.all(pendingBufferViews).then(function(bufferViews) {
      ...
      var byteStride = accessorDef.bufferView !== undefined ?
      ...
      if (byteStride && byteStride !== itemBytes) {
        var ibCacheKey = 'InterleavedBuffer:' + accessorDef.bufferView + ':' +
      ...
      if (accessorDef.sparse !== undefined) {
        ...
        if (bufferView !== null) {
          // Avoid modifying the original ArrayBuffer, if the bufferView wasn't
    ...
    if (source.bufferView !== undefined) {
      ...
      sourceURI = parser.getDependency('bufferView', source.bufferView)
                      .then(function(bufferView) {
                        isObjectURL = true;
                        var blob =
                            new Blob([bufferView], {type: source.mimeType});
  ...
  GLTFParser.prototype.assignTexture = function(
      materialParams, mapName, mapDef) {
    var parser = this;

    return this.getDependency('texture', mapDef.index).then(function(texture) {
      if (parser.extensions[EXTENSIONS.KHR_TEXTURE_TRANSFORM]) {
        var transform = mapDef.extensions !== undefined ?
  ...
  GLTFParser.prototype.loadMaterial = function(materialIndex) {
    ...
    if (alphaMode === ALPHA_MODES.BLEND) {
      ...
      if (alphaMode === ALPHA_MODES.MASK) {
        materialParams.alphaTest = materialDef.alphaCutoff !== undefined ?
    ...
    if (materialDef.emissiveFactor !== undefined &&
      ...
      materialParams.emissive =
          new THREE.Color().fromArray(materialDef.emissiveFactor);
    }
    ...
    return Promise.all(pending).then(function() {
      ...
      // baseColorTexture, emissiveTexture, and specularGlossinessTexture use
      // sRGB encoding.
      if (material.map)
  ...
  function addPrimitiveAttributes(geometry, primitiveDef, parser) {
    ...
    for (var gltfAttributeName in attributes) {
      ...
      pending.push(assignAttributeAccessor(
   ...
   * If we can build a single BufferGeometry with .groups from multiple
   * primitives, returns one BufferGeometry. Otherwise, returns BufferGeometries
   * without .groups as many as primitives.
   *
   * @param {Array<GLTF.Primitive>} primitives
  ...
  GLTFParser.prototype.loadGeometries = function(primitives) {
    ...
    if (isMultiPass) {
      originalPrimitives =
          primitives;  // save original primitives and use later

      // We build a single BufferGeometry with .groups from multiple primitives
      // because all primitives share the same attributes/morph/mode and have
      // indices.
      ...
      if (cached) {
        ...
        if (primitive.extensions &&
          ...
          geometryPromise = addPrimitiveAttributes(
              new THREE.BufferGeometry(), primitive, parser);
        }
    ...
    return Promise.all(pending).then(function(geometries) {
      if (isMultiPass) {
        ...
        var cached =
            getCachedMultiPassGeometry(cache, baseGeometry, originalPrimitives);
        ...
        return Promise.all(pendingIndices).then(function(accessors) {
          ...
          geometries.length > 1 && THREE.BufferGeometryUtils !== undefined) {
        ...
        if (cached) {
          ...
          var geometry =
              THREE.BufferGeometryUtils.mergeBufferGeometries(geometries, true);
    ...
    return Promise.all(pending).then(function(originalMaterials) {
      return parser.loadGeometries(primitives).then(function(geometries) {
        ...
        for (var i = 0, il = geometries.length; i < il; i++) {
          ...
          var material =
              isMultiMaterial ? originalMaterials : originalMaterials[i];
          ...
          for (var j = 0, jl = materials.length; j < jl; j++) {
            ...
            if (mesh.isPoints) {
              ...
              if (!pointsMaterial) {
                ...
                pointsMaterial.lights =
                    false;  // PointsMaterial doesn't support lights yet
              ...
              if (!lineMaterial) {
                ...
                lineMaterial.lights =
                    false;  // LineBasicMaterial doesn't support lights yet
            ...
            if (material.aoMap && geometry.attributes.uv2 === undefined &&
            ...
            if (material.isGLTFSpecularGlossinessMaterial) {
              // for GLTFSpecularGlossinessMaterial(ShaderMaterial) uniforms
              // runtime update
              mesh.onBeforeRender =
                  extensions[EXTENSIONS.KHR_MATERIALS_PBR_SPECULAR_GLOSSINESS]
                      .refreshUniforms;
  ...
  GLTFParser.prototype.loadAnimation = function(animationIndex) {
    ...
    for (var i = 0, il = animationDef.channels.length; i < il; i++) {
      ...
      var input = animationDef.parameters !== undefined ?
          animationDef.parameters[sampler.input] :
          sampler.input;
      var output = animationDef.parameters !== undefined ?
    ...
    return Promise
        ...
        .then(function(dependencies) {
          ...
          for (var i = 0, il = nodes.length; i < il; i++) {
            ...
            var interpolation = sampler.interpolation !== undefined ?
            ...
            // buffers before creating a truncated copy to keep. Because buffers
            // may be reused by other tracks, make copies here.
            for (var j = 0, jl = targetNames.length; j < jl; j++) {
              ...
              // Overrides .createInterpolant in a factory method which creates
              // custom interpolation.
              if (sampler.interpolation === 'CUBICSPLINE') {
                track.createInterpolant =
                    function InterpolantFactoryMethodGLTFCubicSpline(result) {
                  // A CUBICSPLINE keyframe in glTF has three output values for
                  // each input value, representing inTangent, splineVertex, and
                  // outTangent. As a result, track.getValueSize() must be
                  // divided by three to get the interpolant's sampleSize
                  // argument.

                  return new GLTFCubicSplineInterpolant(
                      this.times, this.values, this.getValueSize() / 3, result);
                };

                // Workaround, provide an alternate way to know if the
                // interpolant type is cubis spline to track.
                // track.getInterpolation() doesn't return valid value for
                // custom interpolant.
                track.createInterpolant
          ...
          var name = animationDef.name !== undefined ?
              animationDef.name :
                    ...
                    for (var j = 0, jl = jointNodes.length; j < jl; j++) {
                      ...
                      if (jointNode) {
                        ...
                        if (skinEntry.inverseBindMatrices !== undefined) {
                          mat.fromArray(
                              skinEntry.inverseBindMatrices.array, j * 16);
                        }
