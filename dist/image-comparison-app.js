...
class RenderingScenario extends LitElement {
    ...
    metricTemplate(analysisResults, goldenName) {
        const rmsInDecibel = this.toDecibels(analysisResults[0].rmsDistanceRatio).toFixed(2);
        return html ` <span>${rmsInDecibel} dB </span>
      <div class="tooltip">
        <span class="question-icon"> </span>
        <span class="tooltiptext">
          Root mean square(RMS) color distance between ${goldenName} 
          and current version of model-viewer on rendering ${this.name} in decibels.
